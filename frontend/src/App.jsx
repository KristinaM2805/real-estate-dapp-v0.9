...
function getDefaultExplorerBase(chainId) {
  ...
  if (id === 1) return "https://eth.blockscout.com";
if (id === 11155111) return "https://eth-sepolia.blockscout.com";
  return "";
}
  ...
  return (
    <section className="details" style={{ borderColor: "#bfdbfe", background: "#f8fbff" }}>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 10, marginTop: 14 }}>
        {openAddress ? (
          <a href={openAddress} target="_blank" rel="noreferrer" style={linkStyle}>Открыть контракт в Blockscout</a>
        ) : (
          <span style={disabledStyle}>Blockscout появится в Sepolia</span>
        )}
        {openPayment ? (
...
export default function App() {
  ...
  const sellerNextActionMessage = useMemo(() => {
    ...
    if (stage === 5) return "Покупатель внёс оплату в escrow. Проверьте депозит через Blockscout и создайте заявку в реестр.";
  ...
  const simpleDealMessage = useMemo(() => {
    ...
    if (stage === 5) return `${idText} Оплата внесена в escrow. Продавец должен проверить депозит через Blockscout и разрешить заявку в реестр.`;
  ...
  async function handleReservePayment() {
    ...
    try {
      ...
      setMmStatus("success");
      mmTimer.current = setTimeout(() => setMmStatus("idle"), 3000);
      setMessage("💰 Оплата внесена в escrow. Теперь продавец может проверить депозит через Blockscout и создать заявку в реестр.");
      setAnimPhase(p => p + 1);
      ...
      <aside className="controls controls--side">
        ...
        {isSeller && deal && stage === 5 && (
          <div className="nameInputBox" style={{ gridColumn: "1/-1" }}>
            ...
            <p style={{ margin: "8px 0 12px", color: "#64748b", lineHeight: 1.5 }}>
              Покупатель внёс оплату. Ниже в блоке «Публичная проверка escrow» показаны адрес escrow-контракта, сумма сделки, сумма в escrow и ссылка на Blockscout. В локальной сети ссылка станет активной автоматически после деплоя в Sepolia.
            </p>
            <button
              type="button"
              className="action-btn action-btn--blue"
        ...
        {isSeller && deal && stage >= 2 && stage < 8 && !isCancelled && (
          <div className="nameInputBox" style={{ gridColumn: "1/-1" }}>
            ...
            <p style={{ margin: "6px 0 0", color: "#475569", fontSize: 13, lineHeight: 1.45 }}>
              ...
              {stage === 5 && `Покупатель ${buyerDisplayName} внёс оплату в escrow. Проверьте депозит через Blockscout и нажмите кнопку создания заявки в реестр.`}
